...
def convert_examples_to_features_sequence(
    ...
    for (index, example) in enumerate(examples):
        ...
        if example.dialogue_id not in features:
            features[example.dialogue_id] = [[input_ids, attention_mask, token_type_ids, label]]
        else:
            features[example.dialogue_id].append([input_ids, attention_mask, token_type_ids, label])

    return features


class BuildDataSetSequence(Data.Dataset):
    """
...
def SequenceDataLoader(dataset, device, batch_size=1, shuffle=False):
    ...
    def _collate_fn(batch):
        ...
        labels_ids = [item[3] for item in dialogue_list]
